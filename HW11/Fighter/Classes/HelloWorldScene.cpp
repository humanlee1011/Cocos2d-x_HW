...
bool HelloWorld::init()
{
	...
	time->setPosition(Vec2(origin.x + visibleSize.width / 2, origin.y + visibleSize.height * 3 / 4+ 100));
...
void HelloWorld::DFont_callback(Ref* pSender) {
	int moveX = 0;
	auto location = player->getPosition();
	if (location.x + 50 <= origin.x + visibleSize.width) {
		moveX = 50;
	}
	move(moveX, 0);	
}
...
void HelloWorld::YFont_callback(Ref* pSender) {
...
void HelloWorld::move(int moveX, int moveY) {
	...
	Animation* idleAnimation = Animation::createWithSpriteFrames(idle, 0.1f);
	Animate* originalState = Animate::create(idleAnimation);
	auto moveBy = MoveBy::create(1, Vec2(moveX, moveY));
